...
<body>
    ...
    <div class="main-wrapper">
        ...
        <?php
            ...
            // Handle update request for items
            if (isset($_POST['update'])) {
                $item_id = (int)$_POST['item_id'];  // The item ID to update (cast to int for safety)
                $item_name = $con->real_escape_string($_POST['item_name']);  // The new item name to update
                $price = $con->real_escape_string($_POST['price']);  // Assuming the price is also updated

                // Corrected SQL statement (properly uses WHERE clause)
 ...
 <!--    echo "<td><input type='checkbox' name='selected_items[]' value='" . htmlspecialchars($item_id) . "'></td>";-->
        ...
        <script src="assets/plugins/select2/js/select2.min.js" type="3aab8abce64812a656bcb864-text/javascript"></script>
        ...
        <!-- Show success/error message as SweetAlert -->
        <script>
            document.addEventListener('DOMContentLoaded', function () {
        <?php if ($message): ?>
                    Swal.fire({
