...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($con->query($sql) === TRUE) {
        ...
        $rows = 0;
        $max_items = 1000; // Set a maximum number of iterations to prevent an infinite loop (if needed)
        ...
        while ($rows < $max_items) {
            ...
            // Skip if the item is empty (to avoid inserting blank rows)
            if (empty($item)) {
                $rows++; // Increment row counter to skip this iteration
                continue; // Move to the next iteration
            }

            // Insert valid item into Admin2 table
            $sql2 = "INSERT INTO Admin2 (quotation_id, item, price, qty, total) 
                     VALUES ('$last_id', '$item', '$price', '$qty', '$total')";

            if (!$con->query($sql2)) {
                echo "Error: " . $sql2 . "<br>" . $con->error; // Handle any insert error
            }
