...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($con->query($updateAdminSql) === TRUE) {
        // Update Admin2 data for existing items
        foreach ($admin2Data as $index => $item) {
            ...
            // If the item is not selected in the form, delete it from the database
            if (empty($itemValue) && empty($price) && empty($qty) && empty($total)) {
                ...
                // Update existing item in Admin2
                                                ...
                                                <tbody id="quotationItems">
                                                    <?php foreach ($admin2Data as $index => $item) { ?>
                                                        <tr data-id="<?php echo $item['id']; ?>">
                                                            ...
                                                            <td><input type="number" name="qty[]" class="form-control form-control-mb qty" style="width: 100px;" value="<?php echo htmlspecialchars($item['qty']); ?>" oninput="calculateRowTotal(this)" required></td>
                                                            <td><input type="number" name="total[]" class="form-control form-control-mb total" style="width: 150px;" value="<?php echo htmlspecialchars($item['total']); ?>" oninput="trackManualTotal(this)" required></td>
...
<script>
// Initialize Select2 for dropdowns in #AdminTable with custom matcher function
$('#AdminTable select').select2({
    ...
    matcher: function(params, data) {
        ...
        // Check if the item starts with the search term
        if (data.text.toLowerCase().startsWith(params.term.toLowerCase())) {
            return data;
        }

        // If the item does not match, return null to exclude it
        return null;
    }
});
...
function updatePrice(select) {
    ...
    calculateRowTotal(priceInput);  // Recalculate the total for this row
    calculateTotal();  // Recalculate the overall totals
}

// Calculate total for individual row
function calculateRowTotal(input) {
    ...
    let total = price * qty;
    totalInput.value = total.toFixed(2);  // Set the total to 2 decimal places
    
    calculateTotal();  // Recalculate overall totals after updating the row total
}

// Track manual total input
function trackManualTotal(input) {
    input.dataset.manual = 'true';  // Mark as manually entered
    calculateTotal();  // Recalculate overall totals after manual input
}

// Recalculate all totals (Subtotal, Profit, Loss)
function calculateTotal() {
    ...
    // Update the subtotal and hidden input
    document.getElementById('subtotal').value = subtotal.toFixed(2);
    document.getElementById('subtotalInput').value = subtotal.toFixed(2);

    // Get the quotation amount entered
    const quotationAmount = parseFloat(document.getElementById('quotationAmount').value) || 0;
    const profit = Math.max(0, quotationAmount - subtotal); // Profit calculation
    const loss = Math.max(0, subtotal - quotationAmount); // Loss calculation

    // Update the profit and loss fields
...
function addNewRow() {
    ...
    newRow.innerHTML = `
        ...
        <td><input type="number" name="qty[]" class="form-control form-control-mb qty" style="width: 100px;" oninput="calculateRowTotal(this.closest('tr'))" required /></td>
        <td><input type="number" name="total[]" class="form-control form-control-mb total" style="width: 150px;" oninput="trackManualTotal(this)" required /></td>
...
// Call this function when the page loads to initialize the totals
window.onload = function() {
    calculateTotal(); // Ensure totals are calculated on load
};
</script>
